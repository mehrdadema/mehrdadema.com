...
class CubePortfolioMain {
    ...
    private function getOptionsFromDB() {
        ...
        // set preload option; also guard against an empty array saved by an older version.
        if ( empty( self::$settings['preload'] ) ) {
    ...
    public function generate_cbp($id) {
        if ( $id <= 0 ) {
            return CubePortfolioMain::frontend_error('Cube Portfolio: missing or invalid id attribute in shortcode.');
        }

        $db_data = $this->get_data_from_db($id);

        if ($db_data === NULL) {
            $db_err = $this->wpdb->last_error ? ' (DB: ' . esc_html( $this->wpdb->last_error ) . ')' : '';
            return CubePortfolioMain::frontend_error( 'Incorrect Cube Portfolio ID or database query error. (1001) &mdash; no portfolio found with id=' . (int) $id . $db_err );
        }
        ...
        if ( get_post_type() == self::$settings['postType'] && is_single()) {
            ...
            if (!$this->request_from_ajax) {
                // init standalone cbp post
                 ...
                 wp_register_script('cubeportfolio-jquery-js-standalone', CUBEPORTFOLIO_URL . 'public/js/init-cbp-standalone.min-1.15.1.js', array('cubeportfolio-jquery-js'), CUBEPORTFOLIO_VERSION, true);
