...
class CubePortfolioBackend {
    ...
    private $cubeportfolio_pages = array('cubeportfolio', 'cubeportfolio-settings', 'cubeportfolio-import-export');
    ...
    function __construct() {
        ...
        // Handle form POST/GET actions (replaces the old SPA AJAX for admin UI)
    ...
    public function admin_menu() {
        add_menu_page('Cube Portfolio', 'Cube Portfolio', 'publish_posts', 'cubeportfolio', array( &$this, 'create_home_page' ), CUBEPORTFOLIO_URL . 'admin/img/icon.png');
        ...
        add_submenu_page('cubeportfolio', 'Import / Export', 'Import / Export',    'manage_options', 'cubeportfolio-import-export',   array( &$this, 'create_import_export_page' ) );
    }

    /** Routes to portfolio list or edit view (replaces the old SPA root div). */
    public function create_home_page() {
        ...
        if ( in_array( $action, array( 'edit', 'new' ), true ) ) {
            ...
            include CUBEPORTFOLIO_PATH . 'admin/views/portfolio-edit.php';
        } else {
            $portfolios = $this->wpdb->get_results( 'SELECT * FROM ' . CubePortfolioMain::$table_cbp . ' ORDER BY id' );
            include CUBEPORTFOLIO_PATH . 'admin/views/portfolio-list.php';
        }
    }

    public function create_settings_page() {
        $settings = CubePortfolioMain::$settings;
        include CUBEPORTFOLIO_PATH . 'admin/views/settings.php';
    }

    public function create_import_export_page() {
        include CUBEPORTFOLIO_PATH . 'admin/views/import-export.php';
    }
    ...
    /** Handle form POSTs and GET actions (delete/clone/toggle/export) from the PHP admin. */
    public function handle_admin_actions() {
        if ( ! isset( $_REQUEST['cbp_action'] ) ) {
            // GET-based actions (delete, clone, toggle, export).
            $get_action = isset( $_GET['action'] ) ? sanitize_key( $_GET['action'] ) : '';
            $id         = isset( $_GET['id'] )     ? (int) $_GET['id']               : 0;

            if ( ! $id || ! in_array( $get_action, array( 'delete', 'clone', 'toggle', 'export' ), true ) ) return;

            switch ( $get_action ) {
                case 'delete':
                    check_admin_referer( 'cbp_delete_' . $id );
                    $this->wpdb->delete( CubePortfolioMain::$table_cbp,       array( 'id' => $id ), array( '%d' ) );
                    $this->wpdb->delete( CubePortfolioMain::$table_cbp_items, array( 'cubeportfolio_id' => $id ), array( '%d' ) );
                    wp_redirect( admin_url( 'admin.php?page=cubeportfolio&deleted=1' ) ); exit;

                case 'clone':
                    check_admin_referer( 'cbp_clone_' . $id );
                    $this->clone_portfolio_php( $id );
                    wp_redirect( admin_url( 'admin.php?page=cubeportfolio&cloned=1' ) ); exit;

                case 'toggle':
                    check_admin_referer( 'cbp_toggle_' . $id );
                    $current = (int) $this->wpdb->get_var( $this->wpdb->prepare( 'SELECT active FROM ' . CubePortfolioMain::$table_cbp . ' WHERE id = %d', $id ) );
                    $this->wpdb->update( CubePortfolioMain::$table_cbp, array( 'active' => $current ? 0 : 1 ), array( 'id' => $id ) );
                    wp_redirect( admin_url( 'admin.php?page=cubeportfolio' ) ); exit;

                case 'export':
                    check_admin_referer( 'cbp_export' );
                    require_once CUBEPORTFOLIO_PATH . 'php/CubePortfolioExport.php';
                    new CubePortfolioExport(); // streams JSON and exits.
            }
            return;
        }
        ...
        switch ( $cbp_action ) {
            case 'save_portfolio':
                ...
                if ( ! current_user_can( 'publish_posts' ) ) wp_die( 'Permission denied.' );
                $pid = $this->handle_save_portfolio_php();
                wp_redirect( admin_url( 'admin.php?page=cubeportfolio&action=edit&id=' . $pid . '&saved=1' ) ); exit;

            case 'save_settings':
                check_admin_referer( 'cbp_save_settings', 'cbp_nonce' );
                if ( ! current_user_can( 'manage_options' ) ) wp_die( 'Permission denied.' );
                $this->handle_save_settings_php();
                wp_redirect( admin_url( 'admin.php?page=cubeportfolio-settings&saved=1' ) ); exit;

            case 'import':
                check_admin_referer( 'cbp_import', 'cbp_nonce' );
                if ( ! current_user_can( 'manage_options' ) ) wp_die( 'Permission denied.' );
                $error = $this->handle_import_php();
                if ( $error ) {
    ...
    private function handle_save_portfolio_php() {
        $pid  = (int) ( $_POST['cbp_portfolio_id'] ?? 0 );
        $name = sanitize_text_field( $_POST['cbp_name'] ?? '' );

        // CSS stored as JSON array of lines.
        if ( isset( $_POST['cbp_customcss_text'] ) ) {
            $lines     = array_map( 'trim', explode( "\n", wp_unslash( $_POST['cbp_customcss_text'] ) ) );
            $customcss = wp_json_encode( $lines );
        } else {
            $customcss = wp_json_encode( array() );
        }

        // Parse current ratio + layout from submitted form fields.
        $ratio      = sanitize_text_field( $_POST['cbp_ratio']       ?? 'masonry' );
        $layoutMode = sanitize_text_field( $_POST['cbp_layout_mode'] ?? 'mosaic'  );

        // Rebuild options JSON with the chosen layoutMode.
        $options_raw = wp_unslash( $_POST['cbp_options'] ?? '{}' );
        $options_arr = json_decode( $options_raw, true ) ?: array();
        $options_arr['layoutMode'] = $layoutMode;
        $options = wp_json_encode( $options_arr );

        // Rebuild template with the chosen ratio class.
        $template = wp_unslash( $_POST['cbp_template'] ?? '' );
        if ( $template ) {
            $template = preg_replace( '/cbp-ratio-[\w-]+/', 'cbp-ratio-' . $ratio, $template );
        }

        $data = array(
            'name'         => $name ?: ( $pid ? 'Portfolio #' . $pid : 'New Portfolio' ),
            'type'         => $layoutMode,
            'customcss'    => $customcss,
            'options'      => $options,
            'template'     => $template,
        ...
        if ( $pid ) {
            // Preserve popup config and active state on edit.
            $existing = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT popup, active FROM ' . CubePortfolioMain::$table_cbp . ' WHERE id = %d', $pid ), ARRAY_A );
            $data['popup']  = $existing['popup']  ?? '[]';
            $data['active'] = (int) ( $existing['active'] ?? 1 );
            $this->wpdb->update( CubePortfolioMain::$table_cbp, $data, array( 'id' => $pid ) );
        } else {
            $data['popup']  = '[]';
            $data['active'] = 1;
            $this->wpdb->insert( CubePortfolioMain::$table_cbp, $data );
            $pid = $this->wpdb->insert_id;
            // Patch cbpw-wrapN / cbpw-gridN placeholders now that we have a real ID.
            foreach ( array( 'filtershtml', 'template', 'customcss', 'options' ) as $f ) {
                foreach ( array( 'filters', 'grid', 'loadMore', 'wrap', 'singlePage' ) as $p ) {
                    $data[ $f ] = str_replace( 'cbpw-' . $p . '0', 'cbpw-' . $p . $pid, $data[ $f ] ?? '' );
                }
            }
            $this->wpdb->update( CubePortfolioMain::$table_cbp, array(
                'name' => $data['name'], 'template' => $data['template'],
                'filtershtml' => $data['filtershtml'], 'customcss' => $data['customcss'],
                'options' => $data['options'],
            ), array( 'id' => $pid ) );
        }

        // Sync items.
        $posted_items = ! empty( $_POST['cbp_items_json'] )
            ? ( json_decode( wp_unslash( $_POST['cbp_items_json'] ), true ) ?: array() )
            : ( isset( $_POST['cbp_items'] ) ? (array) $_POST['cbp_items'] : array() );

        $existing_ids = array_column(
            $this->wpdb->get_results( $this->wpdb->prepare( 'SELECT id FROM ' . CubePortfolioMain::$table_cbp_items . ' WHERE cubeportfolio_id = %d', $pid ), ARRAY_A ),
            'id'
        );
        $seen = array();
        ...
        foreach ( $posted_items as $idx => $item ) {
            ...
            if ( $item_id && in_array( $item_id, $existing_ids, false ) ) {
                $this->wpdb->update( CubePortfolioMain::$table_cbp_items, $item_data, array( 'id' => $item_id ) );
                $seen[] = $item_id;
            } else {
                $this->wpdb->insert( CubePortfolioMain::$table_cbp_items, $item_data );
                $seen[] = $this->wpdb->insert_id;
            }
        }
        foreach ( $existing_ids as $eid ) {
            if ( ! in_array( (int) $eid, $seen, true ) ) {
                $this->wpdb->delete( CubePortfolioMain::$table_cbp_items, array( 'id' => $eid ) );
            }
        }
    ...
    private function handle_save_settings_php() {
        $allowed_preload = array( 'onAllPages', 'onHomePage', 'onPostsPage' );
        $raw_preload     = (array) ( $_POST['cbp_preload'] ?? array() );
        $preload         = array_values( array_intersect( array_map( 'sanitize_key', $raw_preload ), $allowed_preload ) );

        // Always keep at minimum "load on pages with shortcode" so the plugin
        // doesn't silently stop working when all boxes are unchecked.
        if ( empty( $preload ) ) {
            $preload = array( 'onPostsPage' );
        }
        ...
        // Merge into the existing settings so we never drop keys like flush_rewrite_rules.
        $settings = get_option( 'cubeportfolio_settings', array() );
        if ( ! is_array( $settings ) ) {
            $settings = array();
        }

        $old_post_type = $settings['postType'] ?? 'cubeportfolio';

        $settings['preload']  = $preload;
        $settings['postType'] = $post_type;

        // If the post type slug changed, schedule a rewrite-rules flush.
        if ( $post_type !== $old_post_type ) {
    ...
    private function handle_import_php() {
        ...
        if ( 'json' !== $ext ) return 'Only .json files are accepted.';

        $raw  = file_get_contents( $_FILES['cbp_import_file']['tmp_name'] );
        $data = json_decode( $raw, true );
        if ( ! $data ) return 'Invalid or empty JSON file.';

        // Re-use the original import logic.
        require_once CUBEPORTFOLIO_PATH . 'php/CubePortfolioImport.php';
        $imp  = new CubePortfolioImport();
        $home = get_home_url();

        $items = $data['cbp_items'] ?? array();
        $cbp   = $data['cbp']       ?? array();
        ...
        foreach ( $items as $k => $item ) {
            ...
            if ( ! empty( $m[0] ) ) {
                foreach ( $m[1] as $i => $p ) {
                    $post = get_post( (int) $p );
                    $m[1][$i] = $post ? ( $post->post_type === 'attachment' ? wp_get_attachment_url($p) : get_permalink($p) ) : '';
                }
                $items[$k]['items'] = str_replace( $m[0], $m[1], $item['items'] );
                $items[$k]['items'] = str_replace( '{{home_url}}', $home, $items[$k]['items'] );
            }
        }
        foreach ( $cbp as $k => $portfolio ) {
            preg_match_all( '/\{\{post_id (.*?)\}\}/', $portfolio['popup'] ?? '', $m );
            if ( ! empty( $m[0] ) ) {
                foreach ( $m[1] as $i => $p ) { $m[1][$i] = get_permalink( (int) $p ) ?: ''; }
                $cbp[$k]['popup'] = str_replace( $m[0], $m[1], $portfolio['popup'] );
                $cbp[$k]['popup'] = str_replace( '{{home_url}}', $home, $cbp[$k]['popup'] );
            }
        }
        if ( isset( $data['settings'] ) ) update_option( 'cubeportfolio_settings', $data['settings'] );

        global $wpdb;
        foreach ( $cbp as $record ) {
            if ( ! $wpdb->get_var( 'SELECT id FROM ' . CubePortfolioMain::$table_cbp . ' WHERE id = ' . (int) $record['id'] ) ) {
                $wpdb->insert( CubePortfolioMain::$table_cbp, $record );
            }
        }
        foreach ( $items as $item ) {
            if ( ! $wpdb->get_var( 'SELECT id FROM ' . CubePortfolioMain::$table_cbp_items . ' WHERE id = ' . (int) $item['id'] ) ) {
                $wpdb->insert( CubePortfolioMain::$table_cbp_items, $item );
            }
        }
        return '';
    }
    ...
    private function get_portfolio_for_edit( $id ) {
        $row = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT * FROM ' . CubePortfolioMain::$table_cbp . ' WHERE id = %d', $id ), ARRAY_A );
        if ( ! $row ) return null;
        $row['items'] = $this->wpdb->get_results( $this->wpdb->prepare( 'SELECT * FROM ' . CubePortfolioMain::$table_cbp_items . ' WHERE cubeportfolio_id = %d ORDER BY sort', $id ), ARRAY_A );
        return $row;
    }

    private function clone_portfolio_php( $id ) {
        $record = $this->wpdb->get_row( $this->wpdb->prepare( 'SELECT * FROM ' . CubePortfolioMain::$table_cbp . ' WHERE id = %d', $id ), ARRAY_A );
        if ( ! $record ) return;
        unset( $record['id'] );
        $record['name'] = 'Copy of ' . $record['name'];
        $this->wpdb->insert( CubePortfolioMain::$table_cbp, $record );
        $new_id = $this->wpdb->insert_id;
        $items  = $this->wpdb->get_results( $this->wpdb->prepare( 'SELECT * FROM ' . CubePortfolioMain::$table_cbp_items . ' WHERE cubeportfolio_id = %d', $id ), ARRAY_A );
        foreach ( $items as $item ) {
            unset( $item['id'] );
            $item['cubeportfolio_id'] = $new_id;
            $this->wpdb->insert( CubePortfolioMain::$table_cbp_items, $item );
        }
    }

    public function register_assets() {
        wp_register_style(  'cubeportfolio-jquery-css', CUBEPORTFOLIO_URL . 'public/css/main.min-1.15.1.css', false, CUBEPORTFOLIO_VERSION, 'all' );
        wp_register_script( 'cubeportfolio-jquery-js',  CUBEPORTFOLIO_URL . 'public/js/main.min-1.15.1.js',  array('jquery'), CUBEPORTFOLIO_VERSION, true );

        // PHP-rendered admin UI styles (replaces the broken SPA admin JS).
        wp_register_style(  'cubeportfolio-admin-ui-css', CUBEPORTFOLIO_URL . 'admin/css/admin-ui.css', false, CUBEPORTFOLIO_VERSION, 'all' );

        // Original admin CSS (colours / icons) kept as-is.
        wp_register_style(  'cubeportfolio-admin-css', CUBEPORTFOLIO_URL . 'admin/css/main.min-1.15.1.css', false, CUBEPORTFOLIO_VERSION, 'all' );
    ...
    public function enqueue_assets() {
        ...
        wp_enqueue_style( 'cubeportfolio-admin-ui-css' );
