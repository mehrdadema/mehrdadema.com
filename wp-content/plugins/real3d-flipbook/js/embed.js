(function($) {
	$(document).ready(function(){
		var initializedIds = {};
		(function run(){
			$.each($(".real3dflipbook"), function(){
				var id = $(this).attr('id');
				if(!id || initializedIds[id] || !$(this).is(':visible')) return;
				initializedIds[id] = true;
				(function(){ // wrap to keep original structure intact
					// console.log(this)
					var id = $(this).attr('id')
					// var optionsName = 
				...
				})(); // end per-book wrapper
			});
			setTimeout(run, 1000); // keep polling for dynamically added books
		})();
	});
}(jQuery));