	...
	function espbw_register_menu() {
		...
		if( ! empty( $wpos_espbw_module ) ) {
			foreach ($wpos_espbw_module as $module_key => $module_val) {
				...
				// Dashboard Page
				add_submenu_page( $module_val['menu'], __('Essential Plugins Bundle By WP OnlineSuport', 'espbw'), '<span style="color:#ff6600;">'.__('Install Our Popular Plugins', 'espbw').'</span>', 'manage_options', "{$module_val['prefix']}-espbw-dashboard", array($this, 'espbw_dashboard_page'), $module_val['position'] );
			}
		}
	}
